...
try{
  ...
  const { data } = await supabase.storage.from('profile-cover').getPublicUrl(`${address}`);
  ...
  if(!data.publicUrl){
    ...
    await supabase.from('dao_members').update({photoURL: data.publicUrl ? data.publicUrl : objectData.photoURL, nickname: objectData.nickname}).eq('userWalletAddress', address);
...
{objectData && objectData.photoURL && !customCover && <Image alt={'avatar'} src={objectData.photoURL} width={160} height={160} className='rounded-full w-36 h-36'/>}
 {objectData && objectData.photoURL && customCover ? <img src={customCover} className='w-36 h-36 rounded-full object-cover'/> : <FaImage className='text-(--hacker-green-4) text-6xl'/>}
