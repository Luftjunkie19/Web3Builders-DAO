...
type Props = {data:`0x${string}`}

function ProposalElementWrapper({data}: Props) {
...
<p  className=' text-xs  text-white'>{fullProposalObject as any && (fullProposalObject as any) && new Date(Number(((fullProposalObject as any).endBlockTimestamp)) * 1000).getTime() >= new Date().getTime() ? '🔓 Open' :  "🔒 Closed"}</p>
      ...
      <Link href={`/proposal/${(fullProposalObject as any).id}`} className="w-full h-full px-4 flex flex-col gap-3 py-2 text-white text-sm overflow-x-hidden">
...
export default ProposalElementWrapper